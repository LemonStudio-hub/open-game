...
impl EventBus {
    ...
    pub fn unsubscribe(&mut self, id: SubscriptionId) {
        for channel in self.channels.values_mut() {
            if let Some(pos) = channel.subscriptions.iter().position(|s| s.id == id) {
                channel.subscriptions.remove(pos);
                return;
            }
        }
    }
    ...
    pub fn emit<T: 'static>(&mut self, event: &T) {
        if let Some(channel) = self.channels.get_mut(&TypeId::of::<T>()) {
            for sub in &mut channel.subscriptions {
                (sub.callback)(event);
            }
        }
    }
...
mod tests {
    ...
    fn test_unsubscribe() {
        ...
        let _id1 = bus.subscribe::<i32>(move |_: &i32| {
            c1.set(c1.get() + 1);
        });
        let id2 = bus.subscribe::<i32>(move |_: &i32| {
            c2.set(c2.get() + 10);
        });
        ...
        bus.unsubscribe(id2);
        bus.emit(&1);
        assert_eq!(count.get(), 12); // Only first subscriber fires
    }
}
